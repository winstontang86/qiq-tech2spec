...
>
> 本模板共 **10 个主体章节 + 附录 A（交付自检）**：1–10 为必填主体章节；附录 A 承载交付自检。任一主体章节缺失或必填表格为空 → 规格书交付不合格。
>
> **无遗留待确认（硬约束）**：所有需人工拍板的参数、以及原始 tech/prd 之间的矛盾点，均已在 skill 执行前的「执行前预检门禁」阶段由用户确认 / 修正完毕。本规格书正文只承载**已定案**决策，不设“待确认”附录。
>
> **交付闭环（硬约束）**：方案中明确要求的每个功能 / 字段 / 错误码 / 流程 / 异常路径 / 参数都必须在本规格书内给出可直接编码的决策。**禁止**出现 `TODO` / `FIXME` / `待补充` / `待定` / `后续 PR` / `下一期补齐` / `留待工程师决定` / `先这样后面优化` / `建议后续补充` 等延迟决策措辞。若生成过程中发现新的信息缺口或矛盾，**不得**在正文留“待确认”，必须回到「执行前预检门禁」向用户澄清确认后再把已定案结论写入正文。
>
...
- 覆盖率报告：`<方案名>-speccover.md`（与本规格书**同目录、同步产出**，以 `<方案名>` 为唯一锚点成对存在）
- 产出时间：{{datetime}}
...
- [ ] 已同步产出 `<方案名>-speccover.md`（与本规格书同目录），未覆盖项 = 0 或已显式豁免（豁免类型仅限“风险接受”或“方案显式不实现”）。
- [ ] **交叉引用可解析**：正文所有 `§x.y` 内部引用的目标章节在本规格书内存在；对原方案的 `§x.y` 引用均指向真实存在的章节（可用 `scripts/check-xref.sh` 校验）。
- [ ] **无遗留待确认**：正文不含“待确认 / 待用户确认 / 待定”等字样；所有原需人工拍板项已在执行前预检阶段确认为定案。
